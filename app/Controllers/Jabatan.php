...
class Jabatan extends BaseController
{
    ...
    {
        $this->breadcrumb->add('Jabatan', '/');

        $data = [
            'title' => getenv('app.name') . ' - Jabatan',
            'page'  => 'Jabatan',
    ...
    public function modalCreate(): string
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'modal' => [
                    'view' => view('jabatan/modal/create', $data),
                    'id' => $id,
                ],
            ];
    ...
    public function modalEdit($jbtn_id): string
    {
        if ($this->request->isAJAX()) {
            ...
            $data['unit_kerja'] = $this->unitKerjaModel->findAll();
            $data['jabatan'] = $this->jabatanModel->join('tbl_md_unit_kerja', 'unkj_id = jbtn_unkj_id')->find($jbtn_id);
    ...
    {
        if ($this->request->isAJAX()) {
            $unitKerja = $this->jabatanModel->find($jbtn_id);
            ...
            try {
                ...
                $this->jabatanModel->update($jbtn_id, $data);
            } catch (\Exception $e) {
                $msg = [
                    'error' => [
                        'msg' => $e->getMessage(),
    ...
    {
        if ($this->request->isAJAX()) {
            $this->jabatanModel->delete($jbtn_id);
    ...
    public function datatables()
    {
        if ($this->request->isAJAX()) {
            ...
            $lists = $this->jabatanModel->getDatatables($params);

            $output = [
                'draw'              => $this->request->getPost('draw'),
                'recordsTotal'      => $this->jabatanModel->countAll(),
                'recordsFiltered'   => $this->jabatanModel->countFiltered($params),
                'data'              => $lists,
                'params'            => $params,
            ];
