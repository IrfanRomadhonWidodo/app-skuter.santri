...
class UserMahasiswa extends BaseController
{
    protected $userModel;
    ...
    public function __construct()
    {
        ...
        $this->userMahasiswaModel   = new \App\Models\UserMahasiswaModel();
    ...
    {
        $this->breadcrumb->add('Mahasiswa', '/');

        $data = [
            'title' => getenv('app.name').' - User Mahasiswa',
            'page'  => 'Data Mahasiswa',
    ...
    {
        $this->breadcrumb->add('Mahasiswa', '/mahasiswa');
        $this->breadcrumb->add('Tambah', '/');

        $data = [
            'title' => getenv('app.name').' - Tambah Mahasiswa',
            'page'  => 'Tambah Mahasiswa',
            'breadcrumbs' => $this->breadcrumb->render(),
            'negara' => $this->negaraModel->orderBy('ngr_nama', 'ASC')->findAll(),
            'agama' => $this->agamaModel->findAll(),
            'program_studi' => $this->programStudiModel->select('prodi_id, prodi_nama, fk_nama')
                                ->join('tbl_md_fakultas', 'fk_id = prodi_fk_id', 'left')->findAll(),
        ];

        return view('user-mahasiswa/create', $data);
    }

    public function edit($usr_id): string
    {
        $this->breadcrumb->add('Mahasiswa', '/mahasiswa');
        ...
        $user = $this->userModel->gets($params, true);

        $data = [
            'title' => getenv('app.name').' - Edit Mahasiswa',
            'page'  => 'Edit Mahasiswa',
            'breadcrumbs' => $this->breadcrumb->render(),
            'negara' => $this->negaraModel->orderBy('ngr_nama', 'ASC')->findAll(),
            'agama' => $this->agamaModel->findAll(),
        ...
        return view('user-mahasiswa/edit', $data);
    }
    ...
    public function save() :string
    {
        if($this->request->isAJAX()) {
            ...
            $data_user = [
                ...
                'usr_status' => 'aktif',
                'usr_registrasi' => '0',
                'usr_otoritas' => 'mahasiswa',
                'usr_created_by' => session()->usr_id,
            ];

            $data_mahasiswa = [
                'usr_mhs_usr_id' => $usr_id,
                'usr_mhs_angkatan' => $this->request->getPost('usr_mhs_angkatan'),
                'usr_mhs_prodi_id' => $this->request->getPost('usr_mhs_prodi_id'),
            ];

            try{
                $this->userModel->insert($data_user);
                $this->userMahasiswaModel->insert($data_mahasiswa);
