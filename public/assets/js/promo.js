...
tabel = $('#table-promo').DataTable({
    ...
    "ajax": {
        "url": $('#table-promo').data('url'),
        "type": $('#table-promo').data('method')
    },
    "deferRender": true,
    "aLengthMenu": [
        [
    ...
    "columns": [
        {
            ...
            "render": function (data, type, row) {
                ...
                data-url="${base_url() +
                        'promo/' + row.prm_id + '/change-status'}" 
                data-method="post" ${data == 1
                    ? 'checked'
                    : ''} 
                id="switch-${row.prm_id}" onchange="return changeStatus('${row.prm_id}')" ${row.prm_edit == false ? 'disabled' : ''}>
                <label class="custom-control-label" for="switch-${row.prm_id}">Aktif</label>
              </div>`;
            }
        }, {
            "data": "prm_id",
            ...
            "render": function (data, type, row, meta) {
                ...
                let btnDelete = `<button class="btn btn-danger btn-sm mb-1 mr-1" data-url="${base_url() + 'promo/' +
                        data + '/delete'}"
