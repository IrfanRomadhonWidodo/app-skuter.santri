    ...
    "columns": [
        {
            ...
            "render": function (data, type, row) {
                return `<img src="${base_url()+'file?file='+ data}" width="150px">`
            }
        }, {
            "data": "hdr_cp_mobile",
            "render": function (data, type, row) {
            ...
            "render": function (data, type, row) {
                ...
                data-url="${base_url() +
                        'header/cek-poin/' + row.hdr_cp_id + '/change-status'}" 
                data-method="post" ${data == 1
                    ? 'checked'
                    : ''} 
                id="switch-${row.hdr_cp_id}" onchange="return changeStatus('${row.hdr_cp_id}')" ${row.prm_edit ? '' : 'disabled'}>
                <label class="custom-control-label" for="switch-${row.hdr_cp_id}">Aktif</label>
              </div>`;
            }
        }, {
            "data": "hdr_cp_id",
            ...
            "render": function (data, type, row, meta) {
                let btnEdit = `<button class="btn btn-success btn-sm mb-1 mr-1" data-url="${base_url() +
                    'header/cek-poin/' + data + '/modal-edit'}"
