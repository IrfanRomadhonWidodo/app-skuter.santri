    ...
    "columns": [
        {
            ...
            "render": function (data, type, row) {
                ...
                data-url="${base_url() +
                        'event/header/' + row.evt_hdr_id + '/change-status'}" 
                data-method="post" ${data == 1
                    ? 'checked'
                    : ''} 
                id="switch-${row.evt_hdr_id}" onchange="return changeStatus('${row.evt_hdr_id}')"
                ${row.prm_edit == false ? 'disabled' : ''}>
                <label class="custom-control-label" for="switch-${row.evt_hdr_id}">Aktif</label>
              </div>`;
            }
        }, {
            "data": "evt_hdr_id",
