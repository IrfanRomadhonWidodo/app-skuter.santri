tabelPencairan = null;
tabelPencairan = $('#table-approver-pencairan').DataTable({
    ...
    "ajax": {
        "url": $('#table-approver-pencairan').data('url'),
        "type": $('#table-approver-pencairan').data('method')
    },
    "deferRender": true,
    "aLengthMenu": [
        [
    ...
    "columns": [
        {
            ...
            "render": function (data, type, row, meta) {
                let btnEdit = `<button class="btn btn-success btn-sm mb-1 mr-1" data-url="${base_url() +
                        'setting/approver-pencairan/' + data + '/modal-edit'}"
...
function reloadDatatablesPencairan() {
    tabelPencairan
        .ajax
        .reload();
}
...
function btnDelete(id) {
    Swal
        .fire({
            text: "Apakah anda yakin akan menghapus approver-pencairan ini?",
            icon: 'warning',
